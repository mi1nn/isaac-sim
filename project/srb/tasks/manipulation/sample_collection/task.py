...
from srb.core.sim import (
    ...
    SphereCfg,
    UsdFileCfg,  # noqa: F401 (swap in for your own USD asset, see `my_object` below)
)
...
class SceneCfg(ManipulationSceneCfg):
    ...
    ## Custom permanent object (respawns with the scene on every reset)
    my_object: RigidObjectCfg = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/my_object",
        spawn=CuboidCfg(
            size=(0.1, 0.1, 0.1),
        ...
        # To use your own USD asset instead, replace the `spawn=` above with:
        # spawn=UsdFileCfg(usd_path="/absolute/path/to/your_asset.usd"),
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0.3, 0.3, 0.1)),
    )
